...
def test_trace_icmp_ipv4_reaches_destination(monkeypatch, capsys):
    tracer = Traceroute("1.1.1.1", protocol="icmp", max_hops=5, queries=2, interval=0)

    def fake_sr1(pkt, verbose=0, timeout=None):
        return IP(src=tracer.target_ip) / ICMP(type=0, id=tracer.packet_id)
...
def test_trace_udp_ipv4_timeouts(monkeypatch, capsys):
    tracer = Traceroute(
        "1.1.1.1", protocol="udp", max_hops=1, queries=2, interval=0, timeout=0.1
    )
